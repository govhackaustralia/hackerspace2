...
(function ($, Drupal) {
  ...
  Drupal.behaviors.corsFileAutoUpload = {
    ...
    attach: function (context, settings) {
      $(context).find('input[type="file"]').once('auto-file-upload').on('change.autoFileUpload', {settings: settings.s3fs_cors, baseUrl: settings.path.baseUrl}, Drupal.S3fsCors.triggerUploadButton);
    },
    detach: function (context, settings, trigger) {
      if (trigger === 'unload') {
        $(context).find('input[type="file"].s3fs-cors-upload').removeOnce('auto-file-upload').off('.autoFileUpload');
      }
    }
  };
   ...
   *   Detaches triggers.
   */
  Drupal.behaviors.corsFilePreviewLinks = {
  ...
  Drupal.S3fsCors = Drupal.S3fsCors || {
    ...
    triggerUploadButton: function (event) {
      ...
      // @FIXME: This seems hacky Id can be
      // When uploading after removing id is edit-field-cors-0-upload--2K85Hs_bSF4
      ...
      $.get(event.data.baseUrl + 'ajax/s3fs_cors_key/' + directory + '/' + file_obj.name, function (data, status) {
        if (status !== 'success') {
        ...
        $.ajax({
          ...
          xhr: function () {
            ...
            if (the_xhr.upload) {
              the_xhr.upload.onprogress = Drupal.S3fsCors.displayProgress;
          ...
          success: function (data, textStatus, jqXHR) {
            file_input.show();
            field_name_array = field_name.slice(1, field_name.length - 1);
            // @TODO: DO we need Delta?
            field_name = field_name_key + '_' + delta;
            // Save file in Drupal
            // file_key like 2016-07/s1.txt contains directory as well as field_name
            $.get(event.data.baseUrl + 'ajax/s3fs_cors_save/' + file_key + '/' + file_obj.size + '/' + field_name, function (data, status) {
              var $fid = data.fid;
              if (!$fid) {
                alert('File couldn\'t be saved in Drupal');
                return;
              }
              // Add the fid to hidden fids
